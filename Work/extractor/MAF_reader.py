        ...
        try:
            if len(mafList) > 0:
                for maf_file in mafList:
                    try:
                        df = pd.read_csv(studyPath + maf_file, sep='\t', dtype='unicode')
                        df = df.astype(str)
                        annotated = df[
                            (df['database_identifier'].notnull()) | (df['metabolite_identification'].notnull())]
                        unknown = annotated[~annotated['database_identifier'].str.startswith(
            ...
            print('Fail to load MAF file from ', studyID, assay)
            continue
...
for maf in res:
    try:
        temp = pd.Series([maf.studyID, maf.annotated, maf.unknown, maf.fullsize - maf.annotated - maf.unknown],
                         index=df.columns)
